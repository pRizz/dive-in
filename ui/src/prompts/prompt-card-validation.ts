...
function sortPromptCards(cards: PromptCardDefinition[]): PromptCardDefinition[] {
  return [...cards].sort(
...
function assertNoDuplicateIds(
  promptCardsWithSources: PromptCardInput[],
  cards: PromptCardDefinition[],
): void {
  const idToSource = new Map<string, string>();
  cards.forEach((card, index) => {
    const sourcePath = promptCardsWithSources[index]?.sourcePath ?? 'unknown source';
...
export function loadPromptCardsFromEntries(entries: PromptCardInput[]): PromptCardDefinition[] {
  const cards = entries.map(({ sourcePath, rawValue }) => validatePromptCard(rawValue, sourcePath));
  assertNoDuplicateIds(entries, cards);
  return sortPromptCards(cards);
}

export function loadPromptCardsFromModules(
  modules: Record<string, unknown>,
): PromptCardDefinition[] {
  const entries = Object.entries(modules).map(([sourcePath, moduleValue]) => ({
    sourcePath,
    rawValue: extractModuleValue(moduleValue),
  }));

  return loadPromptCardsFromEntries(entries);
}
